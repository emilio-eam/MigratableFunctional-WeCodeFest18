...
import we.code.fest.migratable.testing.step5.lib.{SearchOfficeNameService, SearchOfficeService}
...
abstract class SearchOfficeTestBase extends FlatSpec {
  ...
  def getOffice(name: String, country: String, language: String, year: Integer): Office = {
    return new Office(name = name, country = country, language = language, year = year)
  }
